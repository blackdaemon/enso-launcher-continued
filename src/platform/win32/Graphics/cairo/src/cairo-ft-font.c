...
/* HUMANIZED EDIT:
 *
 * There are a number of changes in this file that were made because
 * we needed to be able to use the font rendering of FreeType under
 * win32 (due to the fact that FreeType's font rendering is much nicer
 * than that of the win32 GDI). 
 *
 * Put simply, the cairo philosophy on font usage is different than
 * the one we want to use for Enso: Cairo wants to have access to the
 * system's font registry, whereas we want to be able to simply point
 * cairo to a TrueType font file. The FreeType font driver for cairo
 * uses fontconfig to get information on system fonts; as such, this
 * file has been significantly modified to decouple the font driver
 * from fontconfig. 
 *
 * Accordingly, and somewhat unfortunately, some changes needed to be
 * made to the way cairo's FreeType font api works. All functions that
 * dealt with fontconfig patterns have been removed, and the
 * cairo_select_font_face() function's parameters are now interpreted
 * in the following way:
 *
 *   void
 *   cairo_select_font_face( cairo_t *cr,
 *                           const char *fontFilename,
 *                           cairo_font_slant_t unusedSlant,
 *                           cairo_font_weight_t unusedWeight );
 *
 * cr - The cairo context is used as normal.
 *
 * fontFilename - the path to the font filename to load (e.g.,
 * "arial.ttf").
 *
 * The slant and weight parameters are unused.
 **/
...
#define FC_RGBA_UNKNOWN  1
#define FC_RGBA_RGB      2
#define FC_RGBA_BGR      3
#define FC_RGBA_VRGB     4
#define FC_RGBA_VBGR     5
...
static int
_humanized_check_file_exists( char *filename )
{
...
_cairo_ft_unscaled_font_create_for_pattern (const unsigned char *family)
{
    ...
    filename = (char *) family;
    if ( !_humanized_check_file_exists(filename) )
        goto UNWIND;
				  ...
				  cairo_scaled_font_t	     **font)
{
    ...
    unscaled = _cairo_ft_unscaled_font_create_for_pattern (family);
    if (!unscaled)
        goto UNWIND;

    load_flags = _get_pattern_load_flags ();
